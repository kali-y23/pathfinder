...
static int **parse_matrix(char **strarr, char **set, int n) {
    ...
    int distance = -2;
    char **names_strarr = NULL;
    int index1 = 0;
    int index2 = 0;


    for (int i = 1; strarr[i]; i++) {
        names_strarr = mx_get_names_strarr(strarr[i], &distance);
        index1 = mx_get_str_index(set, names_strarr[0]);
        index2 = mx_get_str_index(set, names_strarr[1]);
...
t_data *mx_parsing(char *filename) {
    ...
    file = mx_file_to_str(filename);
    strarr = mx_strsplit(file, '\n');
